//! Wire DTOs for the `body`/`recipients` JSON columns, shared across the
//! sqlite / D1 / Durable-Object storage adapters. The domain entities don't
//! derive `Serialize`, so this crate owns the single canonical wire format.
